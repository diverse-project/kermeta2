...
class MessagingSystemConsoleReporter (val settings: Settings, reader: BufferedReader, writer: PrintWriter, val messagingSystem: MessagingSystem) extends AbstractReporter {
  def this(settings: Settings, messagingSystem: MessagingSystem) = this(settings, Console.in, new PrintWriter(Console.err, true), messagingSystem)
  ...
  def printMessage(msg: String) { 
    writer.print(msg + "\n"); writer.flush() 
  	messagingSystem.info(msg, "scalac")
  }
