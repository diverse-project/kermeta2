...
trait RepeatAspect extends `logo_compilerNXC`.kmLogo.ASM.ControlStructureAspect with `_root_`.kmLogo.ASM.Repeat{
...
{
`~result` = ((("repeat (").plus(((this).Scalacondition).compileToNXC(context))).plus(")\n")).asInstanceOf[_root_.java.lang.String];
(`~result`).append(((this).Scalablock).compileToNXC(context))}
        }
 return `~result`
}
  override def getMetaClass():_root_.org.kermeta.language.structure.Class={
    var cd : org.kermeta.language.structure.ClassDefinition =   _root_.k2.utils.ReflexivityLoader.getMetaClass("kmLogo.ASM.Repeat"); 
    if (cd !=null){ 
