package logo_compilerNXCrunner 
...
class DefaultRunner  {
  def init() : Unit = {
...

    {
      ...
      pack.setEFactoryInstance(`logo_compilerNXC`.kmLogo.ASM.KerRichFactory)
    }


      k2.utils.ReflexivityLoader.pref("logo_compilerNXC")

      k2.utils.UTilScala.setScalaAspectPrefix("logo_compilerNXC")
  }
...
_root_.kmLogo.ASM.impl.ASMPackageImpl.init().setEFactoryInstance(`logo_compilerNXC`.kmLogo.ASM.KerRichFactory)
