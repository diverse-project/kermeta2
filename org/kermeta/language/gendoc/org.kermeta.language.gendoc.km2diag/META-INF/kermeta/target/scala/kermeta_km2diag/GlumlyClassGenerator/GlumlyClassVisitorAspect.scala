...
trait GlumlyClassVisitorAspect  extends _root_.kermeta_km2diag.transform.Abstract_PackageVisitorAspect with `_root_`.`kermeta_km2diag`.GlumlyClassGenerator.GlumlyClassVisitor{
...
{
((this).ScalafileContent).append((((("H\t").plus((c).Scalaname)).plus("\t")).plus((e).getTypeDef())).plus("\n"))}
}
})
...
{
((this).ScalafileContent).append((((("C\t").plus((c).Scalaname)).plus("\t")).plus(((oa).Scalatype).getTypeDef())).plus("\n"))}
else 


{
((this).ScalafileContent).append((((("S\t").plus((c).Scalaname)).plus("\t")).plus(((oa).Scalatype).getTypeDef())).plus("\n"))}
}
}
})}
})}
  ...
  override def getMetaClass():_root_.org.kermeta.language.structure.Class={
    var cd : org.kermeta.language.structure.ClassDefinition =   _root_.k2.utils.ReflexivityLoader.getMetaClass("GlumlyClassGenerator.GlumlyClassVisitor"); 
    if (cd !=null){ 
