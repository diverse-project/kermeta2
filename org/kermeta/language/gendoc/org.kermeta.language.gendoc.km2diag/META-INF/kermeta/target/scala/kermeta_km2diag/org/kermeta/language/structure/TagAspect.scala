...
trait TagAspect extends `kermeta_km2diag`.org.kermeta.language.structure.KermetaModelElementAspect with `_root_`.org.kermeta.language.structure.Tag{
  ...
  def Scalaobject_=(`~value` : k2.standard.KermetaOrderedSet[_root_.org.kermeta.language.structure.KermetaModelElement]):Unit={
