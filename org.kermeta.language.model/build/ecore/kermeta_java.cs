...
START org.kermeta.language.structure.ModelingUnit
...
TOKENS {
	...
	DEFINE ML_DOCUMENTATION $'/**'.*'*/'$;
	// Multiline comment are /* */ but not /** */
	...
	// a valid identifier is composed of char an digit, can start with ~ and may contain '::' in the middle
	DEFINE IDENTIFIER $('a'..'z'|'A'..'Z'|'_'|'\~') ('a'..'z'|'A'..'Z'|'0'..'9'|'_'|'::')* (('a'..'z'|'A'..'Z'|'0'..'9'|'_'))*$;
	
	//DEFINE QUALIFIEDNAME_IDENTIFIER IDENTIFIER + $($ + QUALIFIEDNAME_SEPARATOR + $ $ + IDENTIFIER + $)* $;
	//DEFINE QUALIFIEDNAME_IDENTIFIER IDENTIFIER + $(('::') $ + IDENTIFIER + $)* $;
