...
trait TypeLiteralAspect extends ObjectAspect {
    ...
    override def generateScalaCode(res : StringBuilder) : Unit = {
        if (this.getTyperef().getType.isInstanceOf[Class]){
            ...
            res.append(_root_.utils.UTilScala.getQualifiedNameTypeKermeta(this.getTyperef().getType, "."))

        }else{
            res.append("_root_." + _root_.utils.UTilScala.getQualifiedNameTypeKermeta(this.getTyperef().getType, "."))
        }
