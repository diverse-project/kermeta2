...
OPTIONS {
	...
	//resourcePluginID = "org.kermeta.language.emftexteditor";
	uiBasePackage = "org.kermeta.language.emftexteditor.ui";
	//resourceUIPluginID = "org.kermeta.language.emftexteditor.ui";
...
TOKENS{
		...
		DEFINE PRIMITIVE_TYPE$'Integer'|'String'|'Boolean'$;
		DEFINE INTEGER$('-')?('1'..'9')('0'..'9')*|'0'$;
		DEFINE FLOAT$('-')?(('1'..'9') ('0'..'9')* | '0') '.' ('0'..'9')+ $;
		DEFINE KERMETA_LITERAL $'kermeta'$;
		...
		//DEFINE NAMESPACEPREFIX IDENTIFIER + $'::'$ + IDENTIFIER;
...
//STRUCTURE
	...
	("package" namespacePrefix[QUALIFIEDNAME] ownedTypeDefinition*)?
	(requires)* (usings)* (packages)*
	 
	;		
      
   	structure.Using ::= 
   	"using" (qualifiedName[QUALIFIEDNAME]|qualifiedName[]|qualifiedName['"','"']) !0
   ;
   	structure.Require ::= 
   	//"require" uri[STRING_LITERAL] !0;
   "require" uri['"','"'] | "require" uri[KERMETA_LITERAL];
...
//best rule declaration but netsingPackage seems not to be resolved   
	...
	"}" 
//avoid limitation above but has side effects
// 	name[] ("::" nestedPackage)* "{"
//    (
//  	   (!0ownedTypeDefinition)+!0 
//  	   |
...
// 	"}" 
	...
	structure.Enumeration ::= 
  	"enumeration" name[] "{" (ownedLiteral";" (ownedLiteral";")*) "}"!0;
   	...
   	structure.PrimitiveType::= name[PRIMITIVE_TYPE] ;
   	
   	structure.UnresolvedType ::= typeIdentifier[] ;
   	
   	structure.VoidType::= "Void" ; 
...
//BEHAVIOUR
	...
	"then" 
			(!1thenBody:bv.CallFeature|thenBody:bv.CallSuperOperation|thenBody:bv.CallResult|
			thenBody:bv.CallVariable|thenBody:bv.CallValue|thenBody:bv.Assignment|thenBody:bv.Block|thenBody:bv.VariableDecl)!0 
	("else" !1elseBody:bv.CallFeature|elseBody:bv.CallSuperOperation|elseBody:bv.CallResult|
			elseBody:bv.CallVariable|elseBody:bv.CallValue|elseBody:bv.Assignment|elseBody:bv.Block|elseBody:bv.VariableDecl)?!0 
	"end" ; 
   ...
   //decision to supress ~ (cf.francoisf)
   //("~")?"result" ;
   "result" ;
 	...
 	"("(target:bv.BooleanLiteral|target:bv.CallVariable|target:bv.JavaStaticCall|target:bv.SelfExpression|target:bv.StringLiteral|target:bv.IntegerLiteral)
   	#0 	("."staticProperty[] | "."staticOperation[]"("(parameters(","parameters)*)?")"|"."staticEnumLiteral[]| "."staticOperation[] )* ("==" parameters ")"|(("+"|"-"|"/"|"*") parameters)* ")" (("+"|"-"|"/"|"*") parameters)*) |
   	
 	(target:bv.BooleanLiteral|target:bv.CallVariable|target:bv.JavaStaticCall|target:bv.SelfExpression|target:bv.StringLiteral|target:bv.IntegerLiteral)
   	#0 	("."staticProperty[] | "."staticOperation[]"("(parameters(","parameters)*)?")"|"."staticEnumLiteral[]| "."staticOperation[] )* ("==" parameters|(("+"|"-"|"/"|"*") parameters)*) //| 	
 	
 	//(target:bv.Literal|target:bv.CallVariable|target:bv.JavaStaticCall|target:bv.SelfExpression)
	...
	bv.JavaStaticCall::= "extern" jclass[] #0 "." jmethod[] #0 "("(parameters(","parameters)*)?")" ;
