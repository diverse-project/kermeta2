 ...
 */
public class KermetaOccurrence {
	...
	private org.kermeta.language.emftexteditor.ui.KermetaTokenScanner tokenScanner;
	 ...
	 */
	public KermetaOccurrence(org.kermeta.language.emftexteditor.IKermetaTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.kermeta.language.emftexteditor.ui.KermetaTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.kermeta.language.emftexteditor.mopp.KermetaMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.kermeta.language.emftexteditor.ui.KermetaBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, org.kermeta.language.emftexteditor.ui.KermetaPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, org.kermeta.language.emftexteditor.ui.KermetaPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
